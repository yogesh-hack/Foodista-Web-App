...
const SeachFilter = ({data}) => {
    ...
    return (
        <form className='w-96'>
           ...
           <div className={(wordentered) ? `h-96 overflow-y-auto absolute bg-white w-96` : 'h-0'}>
           {filterdata.length != 0 && (
                <div className='p-2'>
                    {filterdata.slice(0,15).map((value,key) => {
                        return (
                            <Link to={`/productsdetails/${value.name}`} 
                            key={value.name}
                            onClick={() => {
                                setfilterdata([])
                                setwordentered("")
                            ...
                            className='flex items-center justify-between hover:bg-gray-200'>
                                <img className='mb-3 w-20 h-10' src={value.imageUrl} alt='img'/>
